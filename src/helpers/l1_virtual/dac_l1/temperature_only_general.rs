...
pub struct TsucDependentL1 {
    ...
    tliq_memory_filtered: CircularBufferF64<{ 15 * 60 / RESAMPLING_TIME_USIZE + 1 }>,
...
impl DacL1Calculator for TsucDependentL1 {
    fn calc_l1(
        ...
        let mut conditions = [None; 17];

        conditions[0] = self
            .tsuc_memory_filtered
            .delta(60 / RESAMPLING_TIME_USIZE)
            .map(|delta| delta > -0.35 && tamb - tsuc < 1.5 && tliq - tamb < 6.0);
        ...
        conditions[3] = { Some(tliq - tamb < 7.0 && tamb - tsuc < 1.0 && tsuc > 28.0) };
        ...
        conditions[4] = {
            ...
            let diff = tliq - tamb < 5.0;

            Some(tsuc_delta && tsuc_deltas && diff)
        };

        conditions[5] = { Some((tsuc > tliq || tsuc > tamb) && tsuc > 21.0 && tamb - tsuc < 2.5) };

        conditions[6] = { Some(tamb - tsuc < 0.4 && tliq - tamb < 5.0) };
        ...
        conditions[7] = {
            ...
            let tsuc_deltas_gt_6 = comparison_deltas
                .iter()
                .map(|x| self.tsuc_memory_filtered.delta(*x))
                .any(|p| p.map(|p| p > 6.0).unwrap_or(false));

            let tsuc_deltas_lt_m5 = [60 / RESAMPLING_TIME_USIZE, 5 * 60 / RESAMPLING_TIME_USIZE]
                .iter()
                .map(|x| self.tsuc_memory_filtered.delta(*x))
                .any(|p| p.map(|p| p > -5.0).unwrap_or(false));

            Some(tsuc_deltas_gt_6 && tsuc_deltas_lt_m5)
        };

        conditions[8] = {
            let dtsuc = self.tsuc_memory_filtered.delta(60 / RESAMPLING_TIME_USIZE);
            dtsuc.map(|x| x > -0.35 && tamb - tsuc < 4.0 && tliq - tamb < 2.3)
        };
        ...
        conditions[10] = {
            let dtsuc_gt_035 = self
                ...
                .delta(60 / RESAMPLING_TIME_USIZE)
                .map(|x| x > -0.35);
            dtsuc_gt_035.map(|dts| dts && tamb - tsuc < 4.0 && tliq - tamb < 2.3)
        };

        conditions[11] = { Some(tliq - tamb < 2.5 && tamb - tsuc < 4.0 && tliq - tsuc < 3.5) };
        conditions[12] = {
            let dts_60s = self
                .tsuc_memory_filtered
                .delta(60 / RESAMPLING_TIME_USIZE)
            ...
            match (dts_60s, dts_90s) {
                (Some(d60s), Some(d90s)) => Some((d60s || d90s) && tliq - tamb < 7.5),
                _ => None,
            }
        };
        conditions[13] = {
            ...
            let tsuc_gt_4 = comparison_deltas
                .iter()
                .map(|x| self.tsuc_memory_filtered.delta(*x))
                .any(|p| p.map(|p| p > 4.0).unwrap_or(false));
            ...
            Some(tliq - tamb < 5.0 && tsuc_gt_4 && tsuc_gt_m5)
        };
        conditions[14] = { Some(tamb - tsuc < 6.5 && tliq - tsuc < 1.0) };
        conditions[15] = { Some(tliq - tamb < 11.0 && tamb - tsuc < 1.0 && tsuc > 35.0) };
        conditions[16] = {
            let comparison_deltas = [
                60 / RESAMPLING_TIME_USIZE,
