...
pub async fn read_socket_http_header(
    ...
    let (i_eoh, eoh_len) = loop {
        ...
        if socket.data_len >= 2000 {
            crate::LOG.append_log_tag_msg("ERROR", "ERROR23: Header too long");
...
async fn read_tcp_socket_bytes(socket: &mut SocketReader, timeout_secs: u64) -> Result<(), String> {
    ...
    if socket.data_len == buffer_size {
        crate::LOG.append_log_tag_msg("ERROR", "ERROR23: Insufficient buffer space");
    ...
    .await
    {
        ...
        Ok(v) => match v {
            Err(err) => {
                crate::LOG.append_log_tag_msg("ERROR", &format!("ERROR28 {}", err));
    ...
    if read_count == 0 {
        ...
        crate::LOG.append_log_tag_msg("ERROR", "ERROR344 task_read_client read_count == 0");
