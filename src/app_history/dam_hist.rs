...
use crate::compression::compiler_DAM::DAMTelemetryCompiler;
use crate::lib_http::response::{respond_http_json, respond_http_plain_text};
use crate::lib_http::types::HttpResponse;
use crate::telemetry_payloads::dam_payload_json::get_raw_telemetry_pack_dam;
use crate::telemetry_payloads::dam_telemetry::split_pack;
...
pub async fn process_comp_command_dam(
    ...
    if table_name.len() == 0 {
        crate::LOG.append_log_tag_msg("WARN", &format!("Unknown DAM generation: {}", dev_id));
    ...
    let period_data = match accs.tcomp.CheckClosePeriod(if open_end {
        accs.tcomp.lastIndex + 1
    } else {
        isize::try_from(interval_length_s).unwrap()
    }) {
        Err(err) => {
            crate::LOG.append_log_tag_msg("ERROR", &format!("{}", err));
            return Ok(respond_http_plain_text(400, "ERROR[132] CheckClosePeriod"));
        }
        Ok(v) => match v {
            Some(v) => v,
            None => {
    ...
    data["Temperature"] = period_data.Temperature.into();
    data["Temperature_1"] = period_data.Temperature_1.into();
...
pub fn parse_parameters(parsed: &serde_json::Value) -> Result<ReqParameters, HttpResponse> {
    ...
    if dev_id.len() < 9 {
        return Err(respond_http_plain_text(400, "ERROR[152] dev_id.len() < 9"));
    }
